   ...
   <div class="box-container">
      ...
      <div class="box">
         ...
         if ($select_profile->rowCount() > 0) {
           ...
           $count_user_comments = $conn->prepare(
             "SELECT * FROM `comments` WHERE user_id = ?"
           );
           $count_user_comments->execute([$user_id]);
           $total_user_comments = $count_user_comments->rowCount();

           $count_user_likes = $conn->prepare(
      ...
      <div class="box">
         ...
         <div class="flex-box">
         <?php
         $select_authors = $conn->prepare(
           "SELECT DISTINCT name FROM `admin` LIMIT 10"
         );
         $select_authors->execute();
         if ($select_authors->rowCount() > 0) {
           while (
             $fetch_authors = $select_authors->fetch(PDO::FETCH_ASSOC)
           ) { ?>
            <a href="author_posts.php?author=<?= $fetch_authors[
              "name"
...
<section class="posts-container">
   ...
   <div class="box-container">
      ...
      $select_posts = $conn->prepare(
        "SELECT * FROM `posts` WHERE STATUS = ? order by date DESC LIMIT 6"
      );
      ...
      <form class="box" method="post">
         ...
         <div class="post-content content-150"><?= $fetch_posts[
           "content"
         ] ?></div>
         <a href="view_post.php?post_id=<?= $post_id ?>" class="inline-btn">read more</a>
         <a href="category.php?category=<?= $fetch_posts[
           "category"
         ] ?>" class="post-cat"> <i class="fas fa-tag"></i> <span><?= $fetch_posts[
  "category"
] ?></span></a>
         <div class="icons">
