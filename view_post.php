...
if (isset($_POST["add_comment"])) {
  ...
  $verify_comment = $conn->prepare(
    "SELECT * FROM `comments` WHERE post_id = ? AND admin_id = ? AND user_id = ? AND user_name = ? AND comment = ?"
  );
  ...
  if ($verify_comment->rowCount() > 0) {
    ...
    $insert_comment = $conn->prepare(
      "INSERT INTO `comments`(post_id, admin_id, user_id, user_name, comment) VALUES(?,?,?,?,?)"
    );
...
if (isset($_POST["edit_comment"])) {
  ...
  $verify_comment = $conn->prepare(
    "SELECT * FROM `comments` WHERE comment = ? AND id = ?"
  );
...
<?php if (isset($_POST["open_edit_box"])) {
  ...
  ?>
   ...
   <?php
   $select_edit_comment = $conn->prepare(
   ...
   <form action="" method="POST">
      ...
      <textarea name="comment_edit_box" required cols="30" rows="10" placeholder="please enter your comment"><?= $fetch_edit_comment[
        "comment"
      ] ?></textarea>
...
<section class="posts-container" style="padding-bottom: 0;">
   ...
   <div class="box-container">
      ...
      $select_posts = $conn->prepare(
        "SELECT * FROM `posts` WHERE status = ? AND id = ?"
      );
      ...
      <form class="box" method="post">
         ...
         <div class="post-admin">
            ...
            <div>
               <a href="author_posts.php?author=<?= $fetch_posts[
                 "name"
               ] ?>"><?= $fetch_posts["name"] ?></a>
         ...
         <img src="uploaded_img/<?= $fetch_posts[
           "image"
         ...
         <div class="icons">
            ...
            <button type="submit" name="like_post"><i class="fas fa-heart" style="<?php if (
              $confirm_likes->rowCount() > 0
            ) {
              echo "color:var(--red);";
...
<section class="comments-container">
   ...
   <form action="" method="post" class="add-comment">
      <input type="hidden" name="admin_id" value="<?= $fetch_admin_id[
        "admin_id"
      ] ?>">
      <input type="hidden" name="user_name" value="<?= $fetch_profile[
        "name"
      ] ?>">
      <p class="user"><i class="fas fa-user"></i><a href="update.php"><?= $fetch_profile[
        "name"
      ] ?></a></p>
   ...
   <div class="user-comments-container">
      ...
      <div class="show-comments" style="<?php if (
        $fetch_comments["user_id"] == $user_id
      ) {
        echo "order:-1;";
      } ?>">
         ...
         <div class="comment-box" style="<?php if (
           $fetch_comments["user_id"] == $user_id
         ) {
         ...
         <form action="" method="POST">
            <input type="hidden" name="comment_id" value="<?= $fetch_comments[
              "id"
            ] ?>">
